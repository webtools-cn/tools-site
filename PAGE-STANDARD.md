...
## 二十三、流程进化机制（2026-07-16，不重复犯错）

### 23.1 核心原则

**流程不是写完就固定的，是根据问题不断进化的。**

```
执行流程 → 发现问题 → 记录问题 → 优化流程 → 不再犯
```

犯过一次必须记一次，记了就必须改流程，改了流程就不能再犯。
...
**记录位置：**
- 通用教训 → 写进PAGE-STANDARD.md
- Schema相关 → 更新十二节Schema铁律
- 质检相关 → 更新十七/十八节质检标准
- 生产安全 → 更新十四/十六节安全铁律
- 流量相关 → 更新十九节流量入口
- cron prompt → 更新调度中枢prompt

### 23.3 流程优化触发条件

| 触发条件 | 动作 |
|:---------|:-----|
| 同一问题出现2次 | 写进PAGE-STANDARD.md |
| 同一问题出现3次 | 更新cron prompt + 写进memory |
| Google新报错类型 | 更新Schema铁律 |
| 质检漏检 | 更新质检清单 |
| 用户反馈问题 | 更新质量门 |
| cron执行失败 | 分析原因+优化prompt |

### 23.4 已犯错误清单（持续更新）

| 日期 | 问题 | 根因 | 流程改进 | 状态 |
|:-----|:-----|:-----|:---------|:-----|
| 07-13 | 批量sed改坏全站 | 3个cron同时跑+匹配不精确+没验证 | 禁止批量改已有页面、改前commit、改后验证 | ✅已改 |
| 07-13 | 质检报100%但页面有问题 | 只grep计数不验证效果 | 质检必须深度验证、不能只查有没有 | ✅已改 |
| 07-16 | Schema修了4轮Google还报错 | 猜原因不看Google原文、json.loads≠Google验证 | 修报错看GSC原文、对比正常页面、用Google验证 | ✅已改 |
| 07-16 | 154页缺description | 开发模板缺字段 | Schema模板必须含name+description+reviewCount | ✅已改 |
| 07-16 | 采集了数据没人处理 | 流程开环，没有下家接 | 调度中枢读数据→产生行动→验证闭环 | ✅已改 |
| 07-16 | 社区发帖断了没人管 | 没有持续发帖的机制 | 社区引流cron每3小时跑+跟进回复 | ✅已改 |
| 07-16 | 质检形同摆设 | 只跑脚本不深度验证 | 逐页审核+五维验证+不省token | ✅已改 |

### 23.5 Prompt优化机制

**cron prompt不是写完就不管了，必须根据执行反馈持续优化：**

1. 每次cron执行结果 → 检查是否达成目标
2. 没达成 → 分析为什么 → 优化prompt
3. 重复失败 → 重写prompt关键部分
4. 新教训 → 补充进prompt

**优化记录：**
- 07-16: 调度中枢从"随机开发"改为"数据驱动+三大目标"
- 07-16: 质检从"跑脚本"改为"逐页深度验证"
- 07-16: 新增社区引流cron，防止发帖断档
- 待续...

### 23.6 不重复犯错的保障

1. **问题必须记录** — 不记就会忘，忘了就会再犯
2. **记录必须改流程** — 只记不改等于没记
3. **改了流程必须验证** — 不验证不知道改没改好
4. **验证通过必须固化** — 写进标准/prompt/memory，三处同步
5. **固化后必须执行** — 下次遇到同类问题，按新流程走
