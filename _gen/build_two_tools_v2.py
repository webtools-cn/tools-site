#!/usr/bin/env python3
"""Build 2 new tools using template v3 - batch generation (fixed script)"""
import sys, os
...
# All entries MUST be valid CSS property:value pairs for CSS.supports() to work
# Selectors use "selector(:...)" format
# No at-rules, HTML elements, or APIs (CSS.supports can't test those)
...
CSS_FEATURES_CN = [
    ("布局 Layout", [
        ...
        ("overflow: clip", "裁剪溢出"),
        ("overscroll-behavior: auto", "滚动链控制"),
        ("scroll-snap-type: none", "滚动捕捉"),
        ("scroll-behavior: smooth", "平滑滚动"),
        ("anchor-name: --test", "CSS锚点定位"),
        ("position-anchor: --test", "锚点定位"),
    ]),
    ("动画与过渡 Animation & Transition", [
        ("animation-name: test", "动画"),
        ("animation-timeline: scroll", "滚动驱动动画"),
        ("animation-timeline: view", "视图驱动动画"),
        ("view-timeline-name: --test", "视图时间线"),
        ("scroll-timeline-name: --test", "滚动时间线"),
        ("transition-behavior: allow-discrete", "离散过渡"),
        ("interpolate-size: numeric-only", "尺寸插值"),
    ]),
    ("颜色 Color", [
        ("color: oklch(50% 0.13 180)", "OKLCH颜色空间"),
        ("color: oklab(50% 0 -0.01)", "OKLAB颜色空间"),
        ("color-mix: in srgb red blue", "颜色混合函数"),
        ("light-dark: red blue", "亮暗模式颜色"),
        ("color-contrast: red vs white", "颜色对比度"),
    ]),
    ("排版 Typography", [
        ("text-wrap: pretty", "智能换行"),
        ("text-wrap: balance", "平衡换行"),
        ("text-box-trim: none", "文本框修剪"),
        ("initial-letter: 2", "首字母下沉"),
        ("hyphenate-character: auto", "断字符号"),
        ("font-size: clamp(1rem 2vw 3rem)", "响应式字体"),
        ("font-palette: normal", "字体调色板"),
    ]),
    ("效果与滤镜 Effects & Filters", [
        ("backdrop-filter: blur(5px)", "背景模糊"),
        ("accent-color: red", "强调色"),
        ("scrollbar-color: red blue", "滚动条颜色"),
        ("scrollbar-width: thin", "细滚动条"),
        ("mask-image: none", "遮罩"),
        ("clip-path: circle(50%)", "裁剪路径"),
        ("filter: drop-shadow(0 0 5px black)", "投影滤镜"),
    ]),
    ("选择器 Selectors", [
        ("selector(:has(a))", "父级选择器 :has()"),
        ("selector(:where(a))", "零特异性 :where()"),
        ("selector(:is(a))", "选择器列表 :is()"),
        ("selector(:modal)", "模态框 :modal"),
        ("selector(:user-invalid)", "用户输入 :user-invalid"),
        ("selector(:target-within)", "目标内部 :target-within"),
        ("selector(::view-transition)", "视图过渡 ::view-transition"),
        ("selector(::selection)", "选中文本 ::selection"),
    ]),
    ("响应式 Responsive", [
        ("container-type: inline-size", "容器查询单位"),
        ("width: min(100%, 500px)", "min()函数"),
        ("width: max(50%, 300px)", "max()函数"),
        ("width: clamp(300px 50% 800px)", "clamp()函数"),
        ("aspect-ratio: 16/9", "宽高比"),
        ("object-fit: cover", "对象填充"),
    ]),
    ("CSS新特性 Modern CSS", [
        ("@scope (.card) to (.footer)", "CSS作用域 @scope"),
        ("@layer base", "层叠层 @layer"),
        ("@property --my-color", "自定义属性 @property"),
        ("view-transition-name: test", "视图过渡"),
        ("field-sizing: content", "字段内容自适应"),
        ("content-visibility: auto", "内容可见性"),
    ]),
]
...
CSS_FEATURES_EN = [
    ("Layout", [
        ...
        ("overscroll-behavior: auto", "Scroll chaining"),
        ("scroll-snap-type: none", "Scroll snap"),
        ("scroll-behavior: smooth", "Smooth scrolling"),
        ("anchor-name: --test", "CSS Anchor Positioning"),
        ("position-anchor: --test", "Anchor positioning"),
    ]),
    ("Animation & Transition", [
        ("animation-name: test", "Animations"),
        ("animation-timeline: scroll", "Scroll-driven animations"),
        ("animation-timeline: view", "View-driven animations"),
        ("view-timeline-name: --test", "View timeline"),
        ("scroll-timeline-name: --test", "Scroll timeline"),
        ("transition-behavior: allow-discrete", "Discrete transitions"),
        ("interpolate-size: numeric-only", "Size interpolation"),
    ]),
    ("Color", [
        ("color: oklch(50% 0.13 180)", "OKLCH color space"),
        ("color: oklab(50% 0 -0.01)", "OKLAB color space"),
        ("color-mix: in srgb red blue", "Color mix function"),
        ("light-dark: red blue", "Light/dark mode"),
        ("color-contrast: red vs white", "Color contrast"),
    ]),
    ("Typography", [
        ("text-wrap: pretty", "Pretty text wrap"),
        ("text-wrap: balance", "Balanced text wrap"),
        ("text-box-trim: none", "Text box trim"),
        ("initial-letter: 2", "Drop cap"),
        ("hyphenate-character: auto", "Hyphenation"),
        ("font-size: clamp(1rem 2vw 3rem)", "Responsive font size"),
        ("font-palette: normal", "Font palette"),
    ]),
    ("Effects & Filters", [
        ("backdrop-filter: blur(5px)", "Backdrop blur"),
        ("accent-color: red", "Accent color"),
        ("scrollbar-color: red blue", "Scrollbar color"),
        ("scrollbar-width: thin", "Thin scrollbar"),
        ("mask-image: none", "CSS mask"),
        ("clip-path: circle(50%)", "Clip path"),
        ("filter: drop-shadow(0 0 5px black)", "Drop shadow"),
    ]),
    ("Selectors", [
        ("selector(:has(a))", "Parent :has()"),
        ("selector(:where(a))", "Zero specificity :where()"),
        ("selector(:is(a))", "Selector list :is()"),
        ("selector(:modal)", "Modal :modal"),
        ("selector(:user-invalid)", "User invalid :user-invalid"),
        ("selector(:target-within)", "Target within :target-within"),
        ("selector(::view-transition)", "View transition ::view-transition"),
        ("selector(::selection)", "Selection ::selection"),
    ]),
    ("Responsive", [
        ("container-type: inline-size", "Container query units"),
        ("width: min(100%, 500px)", "min() function"),
        ("width: max(50%, 300px)", "max() function"),
        ("width: clamp(300px 50% 800px)", "clamp() function"),
        ("aspect-ratio: 16/9", "Aspect ratio"),
        ("object-fit: cover", "Object fit"),
    ]),
    ("Modern CSS", [
        ("@scope (.card) to (.footer)", "CSS scoping @scope"),
        ("@layer base", "Cascade layers @layer"),
        ("@property --my-color", "Custom properties @property"),
        ("view-transition-name: test", "View transitions"),
        ("field-sizing: content", "Field sizing"),
        ("content-visibility: auto", "Content visibility"),
    ]),
]

def build_features_js(categories):
    parts = []
    for cat_name, features in categories:
        feats_str = ",".join(
            '["%s","%s"]' % (f[0].replace('"','\\"'), f[1].replace('"','\\"'))
            for f in features
        )
        parts.append('["%s",[%s]]' % (cat_name.replace('"','\\"'), feats_str))
    return "[" + ",".join(parts) + "]"

features_js_cn = build_features_js(CSS_FEATURES_CN)
features_js_en = build_features_js(CSS_FEATURES_EN)
...
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
  ...
  <button class="btn" onclick="toggleCustom()" style="padding:8px 16px;border:1px solid rgba(148,163,184,.3);border-radius:6px;cursor:pointer;font-size:.85rem;background:transparent;color:#94a3b8">&#x2699;&#xFE0F; 自定义测试</button>
...
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
  ...
  <button class="btn" onclick="toggleCustom()" style="padding:8px 16px;border:1px solid rgba(148,163,184,.3);border-radius:6px;cursor:pointer;font-size:.85rem;background:transparent;color:#94a3b8">&#x2699;&#xFE0F; Custom Test</button>
  <span id="statsBadge" style="margin-left:auto;padding:6px 12px;background:#1e293b;border-radius:6px;font-size:.8rem;color:#94a3b8"></span>
</div>
<div id="customTestArea" style="display:none;background:#0f172a;padding:12px;border-radius:8px;margin-bottom:12px">
  <div class="form-group">
...
function renderFeatures(cats) {
  ...
  for (var ci = 0; ci < cats.length; ci++) {
    ...
    for (var fi = 0; fi < cat[1].length; fi++) {
      ...
      var label = status === "yes" ? "\\u2705 \\u652F\\u6301" : (status === "no" ? "\\u274C \\u4E0D\\u652F\\u6301" : "\\u23F3 \\u5F85\\u6D4B\\u8BD5");
      html += "<div class=\\"feat-item\\" data-feature=\\"" + f[0].replace(/"/g,"&quot;") + "\\">";
...
function testSingle(feature) {
  try {
    var idx = feature.indexOf(": ");
    if (idx > 0) {
      var prop = feature.substring(0, idx);
      var val = feature.substring(idx + 2);
      testResults[feature] = CSS.supports(prop, val) ? "yes" : "no";
    } else if (feature.indexOf(":") > 0) {
      var parts = feature.split(":");
      testResults[feature] = CSS.supports(parts[0].trim(), parts.slice(1).join(":").trim()) ? "yes" : "no";
    } else {
...
function updateStats() {
  ...
  if (el) el.textContent = "\\u2705 " + yes + "/" + total + "  \\u2713   \\u274C " + no + "/" + total;
}
...
function runCustomTest() {
  ...
  try {
    ...
    if (supported) {
      el.innerHTML = "<div class=\\"cr-item cr-yes\\">&#x2705; <code>" + display + "</code> &mdash; \\u6D4F\\u89C8\\u5668\\u652F\\u6301\\u6B64\\u7279\\u6027</div>";
    } else {
      el.innerHTML = "<div class=\\"cr-item cr-no\\">&#x274C; <code>" + display + "</code> &mdash; \\u6D4F\\u89C8\\u5668\\u4E0D\\u652F\\u6301\\u6B64\\u7279\\u6027</div>";
    }
  } catch(e) {
    el.innerHTML = "<div class=\\"cr-item cr-no\\">&#x274C; \\u6D4B\\u8BD5\\u5931\\u8D25: " + e.message + "</div>";
  }
}
...
tool1_js_cn = tool1_js_tpl.replace('FEATURES_PLACEHOLDER', features_js_cn).replace('\\u2705 \\u652F\\u6301', '✅ 支持').replace('\\u274C \\u4E0D\\u652F\\u6301', '❌ 不支持').replace('\\u23F3 \\u5F85\\u6D4B\\u8BD5', '⏳ 待测试').replace('\\u2705 ', '✅ ').replace('\\u2713', '✓').replace('\\u274C ', '❌ ')
# Fix the custom test messages
tool1_js_cn = tool1_js_cn.replace('\\u6D4F\\u89C8\\u5668\\u652F\\u6301\\u6B64\\u7279\\u6027', '浏览器支持此特性').replace('\\u6D4F\\u89C8\\u5668\\u4E0D\\u652F\\u6301\\u6B64\\u7279\\u6027', '浏览器不支持此特性').replace('\\u6D4B\\u8BD5\\u5931\\u8D25', '测试失败')

tool1_js_en = tool1_js_tpl.replace('FEATURES_PLACEHOLDER', features_js_en)
# Fix labels for English version
tool1_js_en = tool1_js_en.replace('"\\u2705 \\u652F\\u6301"', '"&#x2705; Supported"').replace('"\\u274C \\u4E0D\\u652F\\u6301"', '"&#x274C; Not Supported"').replace('"\\u23F3 \\u5F85\\u6D4B\\u8BD5"', '"&#x23F3; Pending"')
tool1_js_en = tool1_js_en.replace('"\\u2705 "', '"&#x2705; "').replace('"\\u2713"', '"✓"').replace('"\\u274C "', '"&#x274C; "')
tool1_js_en = tool1_js_en.replace('\\u6D4F\\u89C8\\u5668\\u652F\\u6301\\u6B64\\u7279\\u6027', 'Browser supports this feature').replace('\\u6D4F\\u89C8\\u5668\\u4E0D\\u652F\\u6301\\u6B64\\u7279\\u6027', 'Browser does NOT support this feature').replace('\\u6D4B\\u8BD5\\u5931\\u8D25', 'Test failed')

# But the issue is that the template tool_js gets inserted into template as-is
# The template wraps the tool_js in a <script> tag already
# So I need to make sure the JS is clean without HTML escaping
...
cn1, en1 = builder.build_bilingual(
    ...
    faqs_en=[
        ('How does this tool test CSS features?', 'It uses the browser native CSS.supports() API - no external resources needed, results are 100% accurate.'),
        ...
        ('Are the results accurate?', 'CSS.supports() is a native browser API. Results accurately reflect your current browser actual support.'),
...
function updateMods(e) {
  var ids = ["modCtrl","modAlt","modShift","modMeta"];
  var states = [e.ctrlKey, e.altKey, e.shiftKey, e.metaKey];
  for (var i = 0; i < ids.length; i++) {
    var el = document.getElementById(ids[i]);
    if (states[i]) { modOn(el); } else { modOff(el); }
  }
}
...
function handleKeyEvent(e) {
  ...
  document.getElementById("infoRepeat").textContent = e.repeat ? "Repeat" : "-";
  ...
  var typeColor = e.type === "keydown" ? "#22d3ee" : "#f1c40f";
  ...
  logCount++;
  if (logCount > MAX_LOG && tbody.lastChild) {
...
function toggleLock() {
  ...
  if (locked) {
    ...
    kd.textContent = "PAUSED";
    kd.style.color = "#64748b";
  } else {
    btn.innerHTML = "Pause";
    btn.style.background = "transparent";
    btn.style.color = "#94a3b8";
    kd.style.color = "#f1c40f";
    kd.textContent = "Press any key";
  }
}
...
''',
    faqs_cn=[
        ...
        ('为什么有些按键不显示？', '工具默认不捕获输入框中的按键事件以避免干扰输入。直接点击页面空白区域即可激活按键监听。'),
    ],
    faqs_en=[
        ...
        ('What is the difference between event.key and event.code?', 'event.key returns the character value (e.g. "a", "Enter"), while event.code returns the physical key location (e.g. "KeyA", "Enter") which does not change with keyboard layout.'),
        ('How many log entries are kept?', 'Up to 50 recent keyboard events are kept. Click "Clear Log" to reset.'),
        ('Why do some keys not show up?', 'The tool ignores key events in input fields to avoid interference. Click anywhere on the page to activate keyboard capture.'),
    ],
    seo_cn='<h2>什么是键盘事件测试工具？</h2><p>键盘事件测试工具是一个开发辅助工具，用于实时捕获和显示键盘事件详情。当开发者按下键盘按键时，工具会立即显示event.key、event.code、事件类型、修饰键状态以及按键重复信息。所有数据来自浏览器原生KeyboardEvent API。</p><h2>键盘事件测试工具的用途</h2><p>对于前端开发者来说，理解键盘事件是构建无障碍Web应用的关键。本工具可以帮助您：调试键盘快捷键实现、测试不同键盘布局下的按键行为、验证event.key和event.code的差异、分析修饰键组合的触发逻辑。</p><h2>KeyboardEvent API简介</h2><p>KeyboardEvent是DOM事件接口，用于描述键盘交互。包含keydown（按下）和keyup（释放）两种主要事件类型。常用属性包括key（按键值）、code（物理键位）、ctrlKey/altKey/shiftKey/metaKey（修饰键状态）和repeat（按键重复）。</p>',
    seo_en='<h2>What is a Keyboard Event Tester?</h2><p>A keyboard event tester is a developer tool that captures and displays keyboard event details in real-time. When you press a key, the tool instantly shows event.key, event.code, event type, modifier key states, and key repeat information - all from the browser native KeyboardEvent API.</p><h2>Why Use a Keyboard Event Tester?</h2><p>Understanding keyboard events is essential for building accessible web applications. This tool helps you debug keyboard shortcut implementations, test key behavior across layouts, understand event.key vs event.code differences, and verify modifier key combinations.</p><h2>About the KeyboardEvent API</h2><p>The KeyboardEvent interface describes keyboard interactions. It fires two main event types: keydown (key pressed) and keyup (key released). Key properties include key (character value), code (physical key position), ctrlKey/altKey/shiftKey/metaKey (modifier states), and repeat (auto-repeat).</p>',
)
print(f"  OK: {cn2}")
print(f"  OK: {en2}")
