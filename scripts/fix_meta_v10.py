...
def gen_desc(cn_name, feature):
    ...
    if core and len(core) >= 10:
        ...
        if budget > 10:
            ...
            desc = f"{prefix}，实用的{cn_name}在线解决方案{suffix}"
    else:
        desc = f"{prefix}，实用的{cn_name}在线解决方案，打开浏览器即可使用{suffix}"
    
    # 安全网：太短则扩展suffix
    if len(desc) < 100:
        suffix_long = "。纯前端本地处理，数据不上传服务器，无需注册完全免费。"
        desc = f"{prefix}，{core if core and len(core)>=10 else cn_name+'一站式在线解决方案'}{suffix_long}"
    if len(desc) > 160:
        cut = desc[:160].rfind('。')
